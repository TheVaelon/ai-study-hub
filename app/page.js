  ...
  useEffect(() => {
    ...
    document.getElementById("loginBtn").addEventListener("click", ()=>{
      ...
      if(accounts[u] && accounts[u]===p){
        currentUser=u; isAdmin=(u==="marwan_alsaadi");
    ...
    // CHAT
    function newChat(){const id=Date.now();const c={id,name:"New Chat",messages:[]};chats.push(c);currentChat=id;save();renderChatList();renderMessages();}
    function renderChatList(){const list=document.getElementById("chatList");list.innerHTML="";chats.forEach(c=>{const div=document.createElement("div");div.className="chatItem";div.textContent=c.name;div.onclick=()=>{currentChat=c.id;renderMessages();};list.appendChild(div);});}
    function renderMessages(){const chat=chats.find(c=>c.id===currentChat);const box=document.getElementById("chatBox");box.innerHTML="";if(!chat)return;chat.messages.forEach(m=>{const div=document.createElement("div");div.className="msg "+m.role;div.textContent=m.text;box.appendChild(div);});box.scrollTop=box.scrollHeight;}
    function addMessage(role,text){const chat=chats.find(c=>c.id===currentChat);if(!chat)return;chat.messages.push({role,text});if(chat.name==="New Chat"&&role==="user")chat.name=text.slice(0,15)+"...";save();renderChatList();renderMessages();}
    let stopTyping=false;
    async function typeWriter(text){const chat=chats.find(c=>c.id===currentChat);if(!chat)return;let msg={role:"ai",text:""};chat.messages.push(msg);document.getElementById("stopBtn").style.display="inline-block";for(let i=0;i<text.length;i++){if(stopTyping){msg.text=text;renderMessages();break;}msg.text+=text[i];renderMessages();await new Promise(r=>setTimeout(r,5));}stopTyping=false;document.getElementById("stopBtn").style.display="none";save();}
...
async function webSearch(){
  ...
  try{
    ...
    document.getElementById("question").addEventListener("keydown",e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();window.askAI();}});

    document.getElementById("newChatBtn").addEventListener("click", newChat);

    // DASHBOARD
    function updateDashboard(){document.getElementById("todayDate").textContent="Today: "+new Date().toDateString();if(assignments.length>0){const next=assignments.sort((a,b)=>new Date(a.date)-new Date(b.date))[0];document.getElementById("nextAssignment").textContent=`${next.title} (${next.type}) due ${next.date}`;}const feed=document.getElementById("activityFeed");feed.innerHTML="";activity.slice(-5).reverse().forEach(a=>{let li=document.createElement("li");li.textContent=a;feed.appendChild(li);});}

    // ASSIGNMENTS
    function addAssignment(){const t=document.getElementById("assTitle").value.trim();const ty=document.getElementById("assType").value;const d=document.getElementById("assDate").value;if(!t||!d)return alert("Fill all fields");assignments.push({title:t,type:ty,date:d,user:currentUser});activity.push(`${currentUser} added ${ty} → ${t}`);save();renderAssignments();updateDashboard();}
    function renderAssignments(){const list=document.getElementById("assignmentsList");list.innerHTML="";assignments.forEach((a,i)=>{const div=document.createElement("div");div.className="card";div.innerHTML=`<b>${a.title}</b> (${a.type}) due ${a.date}<br><small>by ${a.user}</small>`;if(isAdmin){const btn=document.createElement("button");btn.textContent="🗑";btn.onclick=()=>{assignments.splice(i,1);save();renderAssignments();updateDashboard();};div.appendChild(btn);}list.appendChild(div);});}
    document.getElementById("addAssignmentBtn").addEventListener("click", addAssignment);

    // ADMIN
    function renderAccounts(){const list=document.getElementById("accountsList");list.innerHTML="";Object.keys(accounts).forEach(u=>{if(u==="marwan_alsaadi")return;const div=document.createElement("div");const del=document.createElement("button");del.textContent="Delete";del.onclick=()=>{delete accounts[u];save();renderAccounts();};div.innerHTML=`<b>${u}</b> `;div.appendChild(del);list.appendChild(div);});}
    document.getElementById("createAccountBtn").addEventListener("click",()=>{if(!isAdmin)return;const u=document.getElementById("newUser").value.trim();const p=document.getElementById("newPass").value.trim();if(!u||!p)return alert("Fill fields");accounts[u]=p;save();renderAccounts();alert("✅ Created "+u);});
